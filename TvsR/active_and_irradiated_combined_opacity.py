...
#density is not yet being passed to kappa. the defualt density is being used
#for all opacity function calls...
def active_and_irradiated_combined_opacity(a, Mstar, Rstar, Tstar,
                                          diskMassFrac=0.01, amin=None,aout=100.,
                                          sampled_ts=np.linspace(10,3000,5),
                                           zero_find_default=10.):
    ...
    dissapation. The equation I use is a modification of eq 18 in Kratter et
    al. 2008, that also incorporates the contribution of the accretion shock
    irradiation.  The equation is:

    \sigmaT_{disk}^4=(((8/6)*Sigma*kappa_r(T))+(1/(2*Sigma*kappa_p(T))))*F_{nu}+F_{irr},

    where F_{nu} is the flux from an active disk defined elsewhere in these
    codes (see Lacc.L_active), and F_{irr}=F_{star}+F_{acc}. kappa_r(T) and
    kappa_p(T) are the  temperature dependent Rosseland and Planck mean
    opacities from Semenov et al. (2003)
    Inputs:
    ...
    Returns:
    (Tr,Mr)      -  The temperature as a function of radius and the accretion rate as 
    ...
    while True:
        ...
        def thefunc(Trim, returnTerms=False):
            Sigmar = Sigmar_func(Mstar_sol*diskMassFrac, amin, beta=-1, 
                                 rin=amin[0], rout=aout)
            Sigmaa = Sigma_active(Sigmar, Trim)
            cs = cfromT(T)
            Mdot_cgs = Mdot(cs, Omega_func(Mstar_sol, amin), Sigmaa)
            F_active = (3/(8*np.pi)) * (Mdot_cgs) * 
                       (Omega_func(Mstar_sol,amin)**2) * 
                       (1-np.sqrt(Rstar_cgs/(amin*au2cm)))
        ...
        print cfromT(rim_temp), Omega_func(Mstar_sol,amin) 
        print Sigma_active(Sigmar_func(Mstar_sol*diskMassFrac, amin, beta=-1,rin=amin[0], rout=aout), rim_temp)
    ...
    for ra in zip(a_au, alphas):
        ...
        def thefunc(T):
            ...
            F_active=(3/(8*np.pi)) * (Mdot_cgs) * (Omega_func(Mstar_sol,ra[0])**2) * 
                      (1-np.sqrt(Rstar_cgs/(ra[0]*au2cm)))
            first_term = (stephan_boltzmann*T**4)
            second_term = (8./6.)*Sigmaa*F_active*kappa(T)
            third_term = ((0.5/Sigmaa)*F_active)/kappa(T,Rosseland=False)
